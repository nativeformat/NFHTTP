...
CachingSQLiteDatabase::CachingSQLiteDatabase(
    ...
    : _delegate(delegate) {
  int sqlite_error =
      sqlite3_open((cache_location + ".nfhttp").c_str(), &_sqlite_handle);
  if (sqlite_error != SQLITE_OK) {
    printf("SQLite failed to open: %d\n", sqlite_error);
  } else {
    // Create our tables
    std::string create_tables_query =
        "CREATE TABLE IF NOT EXISTS " + http_table_name + "(" +
        header_hash_column_name + " STRING PRIMARY KEY NOT NULL, " +
        expiry_column_name + " DATETIME NOT NULL, " + etag_column_name +
        " STRING, " + modified_column_name + " DATETIME NOT NULL, " +
        response_serialised_column_name + " STRING NOT NULL, " +
        last_accessed_column_name + " DATETIME NOT NULL, " +
        file_size_column_name + " INT NOT NULL);";
    create_tables_query +=
        "CREATE TABLE IF NOT EXISTS " + pinned_items_table_name + " (" +
    ...
    sqlite_error = sqlite3_exec(_sqlite_handle, create_tables_query.c_str(),
                                nullptr, this, &error_message);
    if (sqlite_error != SQLITE_OK) {
...
std::string CachingSQLiteDatabase::cachingType() const { return "sqlite"; }
...
void CachingSQLiteDatabase::fetchItemForRequest(
  ...
  caching_callback cache_function =
      [callback, this,
       &executed](const std::unordered_map<std::string, std::string> &map) {
        std::string header_hash = map.at(header_hash_column_name);
        char *error_message = nullptr;
        sqlite3_exec(_sqlite_handle,
                     ("UPDATE " + http_table_name + " SET " +
                      last_accessed_column_name + " = date('now') WHERE " +
                      header_hash_column_name + " = '" + header_hash + "'")
                         .c_str(),
                     nullptr, nullptr, &error_message);

        CacheItem item = {
            timeFromSQLDateTimeString(map.at(expiry_column_name)),
  ...
  int error = sqlite3_exec(
      _sqlite_handle,
      ("SELECT " + header_hash_column_name + ", " + expiry_column_name + ", " +
       etag_column_name + ", " + modified_column_name + ", " +
       response_serialised_column_name + ", " + last_accessed_column_name +
       " FROM " + http_table_name + " WHERE " + header_hash_column_name +
       " = '" + request_identifier + "'")
          .c_str(),
      &sqliteSelectHTTPCallback, &cache_function, &error_message);
...
void CachingSQLiteDatabase::storeResponse(
  ...
  int error = sqlite3_exec(
      ...
      ("REPLACE INTO " + http_table_name + " (" + header_hash_column_name +
       ", " + expiry_column_name + ", " + etag_column_name + ", " +
       modified_column_name + ", " + response_serialised_column_name + ", " +
       last_accessed_column_name + ", " + file_size_column_name +
       ") VALUES ('" + response->request()->hash() + "', " + expiry_value +
       ", '" + etag + "', '" + last_modified + "', '" + response->serialise() +
       "', date('now'), " + std::to_string(data_length) + ");")
          .c_str(),
      &sqliteReplaceHTTPCallback, &cache_function, &error_message);
  if (error != SQLITE_OK || !executed) {
...
void CachingSQLiteDatabase::prune() {
  caching_callback cache_function = [this](const std::unordered_map<
        ...
        [this, current_size](
          ...
          for (const auto &result_map : results) {
            ...
            int error_code = sqlite3_exec(_sqlite_handle,
                                          ("DELETE FROM " + http_table_name +
                                           " WHERE " + header_hash_column_name +
                                           " = '" + header_hash + "'")
                                              .c_str(),
                                          nullptr, nullptr, &error_message);
            ...
            local_current_size -=
                std::stoi(result_map.at(file_size_column_name));
               ...
               this](const std::vector<
                ...
                for (const auto &result_map : results) {
                  std::string header_hash =
                      result_map.at(header_hash_column_name);
                  char *error_message = nullptr;
          ...
          char *error_message = nullptr;
          sqlite3_exec(_sqlite_handle,
                       ("SELECT " + header_hash_column_name + ", " +
                        file_size_column_name + " FROM " + http_table_name +
                        " ORDER BY " + last_accessed_column_name)
                           .c_str(),
                       &sqliteSelectVectorHTTPCallback, &cache_function,
                       &error_message);
        };
    ...
    sqlite3_exec(_sqlite_handle,
                 ("SELECT " + header_hash_column_name + ", " +
                  file_size_column_name + " FROM " + http_table_name +
                  " ORDER BY " + expiry_column_name + " ASC")
                     .c_str(),
                 &sqliteSelectVectorHTTPCallback, &cache_function,
                 &error_message);
  };
  ...
  char *error_message = nullptr;
  sqlite3_exec(_sqlite_handle,
               ("SELECT SUM(" + file_size_column_name + ") AS " +
                current_size_virtual_column_name + " FROM " + http_table_name)
                   .c_str(),
               &sqliteSelectHTTPCallback, &cache_function, &error_message);
}

void CachingSQLiteDatabase::pinItem(const CacheItem &item,
                                    const std::string &pin_identifier) {
  char *error_message = nullptr;
  sqlite3_exec(_sqlite_handle,
               ("REPLACE INTO " + pinned_items_table_name + " (" +
                header_hash_column_name + ", " + pin_identifier_column_name +
                ") VALUES ('" + item.payload_filename + "', '" +
                pin_identifier + "')")
                   .c_str(),
               nullptr, nullptr, &error_message);
}

void CachingSQLiteDatabase::unpinItem(const CacheItem &item,
                                      const std::string &pin_identifier) {
  char *error_message = nullptr;
  sqlite3_exec(_sqlite_handle,
               ("DELETE FROM " + pinned_items_table_name + " WHERE " +
                header_hash_column_name + " = '" + item.payload_filename +
                "' AND " + pin_identifier_column_name + " = '" +
                pin_identifier + "'")
                   .c_str(),
               nullptr, nullptr, &error_message);
}

void CachingSQLiteDatabase::removePinnedItemsForIdentifier(
    const std::string &pin_identifier) {
  char *error_message = nullptr;
  sqlite3_exec(_sqlite_handle,
               ("DELETE FROM " + pinned_items_table_name + " WHERE " +
                pin_identifier_column_name + " = '" + pin_identifier + "'")
                   .c_str(),
               nullptr, nullptr, &error_message);
}
...
void CachingSQLiteDatabase::pinnedItemsForIdentifier(
  ...
  caching_vector_callback sqlite_callback =
      [callback](const std::vector<std::unordered_map<std::string, std::string>>
                     &results) {
        std::vector<CacheItem> items;
        for (const auto &result : results) {
          items.push_back(
              {timeFromSQLDateTimeString(result.at(expiry_column_name)),
               ...
               result.at(header_hash_column_name), true});
        }
        callback(items);
      };
  ...
  sqlite3_exec(
      _sqlite_handle,
      ("SELECT " + http_table_name + "." + expiry_column_name + " AS " +
       expiry_column_name + ", " + http_table_name + "." + etag_column_name +
       " AS " + etag_column_name + ", " + http_table_name + "." +
       modified_column_name + " AS " + modified_column_name + ", " +
       http_table_name + "." + header_hash_column_name + " AS " +
       header_hash_column_name + ", " + http_table_name + "." +
       response_serialised_column_name + " AS " +
       response_serialised_column_name + ", " + http_table_name + "." +
       last_accessed_column_name + " AS " + last_accessed_column_name +
       " FROM " + http_table_name + ", " + pinned_items_table_name + " WHERE " +
       http_table_name + "." + header_hash_column_name + " = " +
       pinned_items_table_name + "." + header_hash_column_name + " AND " +
       pinned_items_table_name + "." + pin_identifier_column_name + " = '" +
       pin_identifier + "'")
          .c_str(),
      &sqliteSelectVectorHTTPCallback, &sqlite_callback, &error_message);
}
...
void CachingSQLiteDatabase::pinningIdentifiers(
  ...
  char *error_message = nullptr;
  sqlite3_exec(_sqlite_handle,
               ("SELECT UNIQUE(" + pin_identifier_column_name + ") FROM " +
                pinned_items_table_name)
                   .c_str(),
               &sqliteSelectVectorHTTPCallback, &sqlite_callback,
               &error_message);
}

int CachingSQLiteDatabase::sqliteSelectHTTPCallback(void *context, int argc,
                                                    char **argv,
                                                    char **column_names) {
...
int CachingSQLiteDatabase::sqliteReplaceHTTPCallback(void *context, int argc,
                                                     char **argv,
                                                     char **column_names) {
  caching_callback *cache_function = (caching_callback *)context;
  (*cache_function)({});
...
std::time_t CachingSQLiteDatabase::timeFromSQLDateTimeString(
