...
CachingClient::CachingClient(const std::shared_ptr<Client> &client,
    ...
    : _client(client),
      _cache_location(cache_location.back() == '/' ? cache_location
                                                   : cache_location + "/") {}
...
void CachingClient::requestTokenDidCancel(
...
std::shared_ptr<RequestToken> CachingClient::performRequest(
  ...
  std::shared_ptr<Request> new_request =
      std::make_shared<RequestImplementation>(*request.get());
  ...
  if (!shouldCacheRequest(new_request)) {
    ...
    request_token = std::make_shared<RequestTokenImplementation>(
        shared_from_this(), request_hash);
    _database->fetchItemForRequest(
        request_hash,
        [request_token, callback, new_request,
         this](CachingDatabase::ErrorCode code, const CacheItem &item) {
          if (request_token->cancelled()) {
            std::shared_ptr<Response> cancelled_response =
          ...
          auto wrapped_callback =
              [callback, item, this,
               request_token](const std::shared_ptr<Response> &response) {
                ...
                if (cache_control.no_store || cache_control.no_cache) {
                  ...
                  case StatusCodePartialContent:
                    _database->storeResponse(
                        response,
                        [callback, item,
                         this](CachingDatabase::ErrorCode code,
                               const std::shared_ptr<Response> &response) {
                          // Write the cached file back to disk
                          std::string filename =
                              item.valid ? item.payload_filename
                                         : response->request()->hash();
                          FILE *cache_file =
                              fopen((_cache_location + filename).c_str(), "w");
                  ...
                  case StatusCodeNotModified:
                    _database->storeResponse(
                        responseFromCacheItem(item, response),
          ...
          if (item.valid) {
            ...
            Response::CacheControl response_cache_control =
                response->cacheControl();
            std::time_t result = std::time(nullptr);
            bool expired =
                difftime(result, item.expiry_time) > cache_control.max_stale;
            if (expired || response_cache_control.must_revalidate) {
              ...
              if (item.etag.length() > 0) {
                ...
                (*new_request)["If-Modified-Since"] =
                    output_string_stream.str();
              }
              _tokens[request_token] =
                  _client->performRequest(new_request, wrapped_callback);
              return;
            } else {
              callback(response);
              _tokens.erase(request_token);
...
void CachingClient::pinResponse(const std::shared_ptr<Response> &response,
  ...
  _database->fetchItemForRequest(
      response->request()->hash(),
      [this, pin_identifier](CachingDatabase::ErrorCode error_code,
...
void CachingClient::removePinnedResponseForIdentifier(
    const std::string &pin_identifier) {
...
void CachingClient::pinnedResponsesForIdentifier(
    ...
    std::function<void(const std::vector<std::shared_ptr<Response>> &)>
        callback) {
  _database->pinnedItemsForIdentifier(
      pin_identifier, [callback, this](const std::vector<CacheItem> &items) {
...
const std::shared_ptr<Response> CachingClient::responseFromCacheItem(
  ...
  FILE *cache_file =
      fopen((_cache_location + item.payload_filename).c_str(), "r");
  size_t data_length = 0;
  ...
  const std::shared_ptr<Response> output_response =
      std::make_shared<ResponseImplementation>(item.response, data, data_length,
                                               response);
  if (data != NULL) {
    free(data);
  }
...
bool CachingClient::shouldCacheRequest(
    const std::shared_ptr<Request> &request) {
...
void CachingClient::initialise() {
  _database =
      createCachingDatabase(_cache_location, "sqlite", shared_from_this());
