...
public:
  CachingClient(const std::shared_ptr<Client> &client,
...
private:
  ...
  const std::shared_ptr<Response> responseFromCacheItem(
      ...
      const std::shared_ptr<Response> &response = nullptr) const;
  ...
  std::unordered_map<std::shared_ptr<RequestToken>, std::weak_ptr<RequestToken>>
      _tokens;
