...
class TestCore652(unittest.TestCase):
    '''
    https://jira.spotify.net/browse/CORE-652: Vulcan fails until manually
    cleaned if a download fails
    '''

    def test_core_652(self):
        ...
        try:
            ...
            zipname = os.path.join(vulcan_folder, 'cache', 'guice-3.0.zip-' +
            ...
            with open(zipname, 'w') as f:
                f.write('corrupted zip file')
            assemble_project(vulcan_folder, project_dir,
                             project, properties)
            self.assertTrue(os.path.getsize(zipname) == 886675)
        finally:
